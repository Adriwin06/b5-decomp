...
// thing UpdateSuspensionSprings pushes against; without it the ReadUpdatedBodies gravity step
// (`mLinearVelocity.y -= KF_GRAVITY*dt`) has nothing opposing it.
//
// THE PRODUCER LIFETIME IS WHOLE AND IT RUNS: StartVehicleTractionLineTests @0x82629CE0,
// AddRaceCarTractionLineTests @0x825E9640, EndVehicleTractionLineTests @0x82633CD8 and
// SimpleVehiclePhysics::GetTractionLine @0x825D85C0. They can only land together: End
// dereferences mpTractionLineStreamProducer with no null guard and UpdateVehiclePhysics reaches
...
namespace
{
    ...
    //
    // THE HALVES ARE CAPTURED IN DIFFERENT FUNCTIONS AND PRINTED ONCE. The line geometry only
    ...
    //
    // IT PRINTS THE CAR INDEX ON EVERY LINE, AND EVERY LIVE CAR. Cars 0 and 1 sit at the junkyard
    // all session and the local player is car 2, so a probe that omits the index can truthfully
    // report a PARKED car while the player drives 165 m away.
    // -----------------------------------------------------------------------------------------
            ...
            CgsMemory::SimpleDataStreamResultIterator* lpResultIterator)
    {
             ...
             liCar = mUsedRaceCars.GetNextNonZeroBit(liCar))
        {
            ...
            for (s32 liWheel = 0; liWheel < eNumDrivenWheels; ++liWheel)
            {
                ...
                // FLAG -- A REAL TYPE FORK, not a shortcut here. Wheel::RoadContact::
    ...
    // NOTE FOR THE LEDGER: this is why TriangleCacheInterface::GetCache @0x82277810 is NOT
    ...
    //
    // AS SHIPPED, THE RETURN IS A COMMAND COUNT, NOT A LINE COUNT: `HIDWORD(v57)` is bumped once
    ...
    //
    // WHAT RUNS TODAY, HONESTLY: the lifetime is real and executes every frame, but
    ...
    //
    // THE CURSOR IS COPIED BY VALUE ONCE (`ld r11, 0x38(producer) ; std r11, <stack>` at
    ...
    //
    // AND THE STREAM IS CLOSED BEFORE ANY HARVEST (0x82633D7C, ahead of the three `bl`s), which
    ...
    void VehicleManager::EndVehicleTractionLineTests(
            ...
            const VehicleInputInterface* /*lpInputInterface*/)
    {
        // The second parameter is genuinely UNREAD by the console body -- r5 is never touched
