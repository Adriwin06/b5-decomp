...
// and it now declares all 65 shot-group slots plus the head members the original
// declarations give, so every
...
// consumer's role; the names on the right are the originals, and the offsets agree):
...
// mBurnoutLicense, and the original's accessor for it is GetBurnoutLicense(), so the
...
// fork's field names map onto the canonical base names BY CONSOLE OFFSET, which both models
// agree on:
...
// Fail, and `SetCantSwitchFromMeNow(..., 29)` lands inside the account's attested
// no-cut-from band [27,31) -- independent confirmation of both the base layout and the
// ValidityAccount bounds.
//
// FLAG (+0x0A): the original declarations name that byte `mbTweakerAttached`, yet this Update
...
//   DELETE-WHEN: BehaviourIceAnim::Update is re-walked against the base layout.
// ----------------------------------------------------------------------------
...
// canonical home has it as the MEMBER `void Construct()` (the console passes the tweaker as
// the implicit this),
...
// original member the fork's own offset comment pinned (e.g. GetWorld -> mpAllVehicleData
...
public:
    ...
    // the original declares it as `typedef const Attrib::RefSpec ShotReference;`), which is what
    ...
    // RefSpec exactly as the console does; see its body.
    ...
    // consumers (MomentStationaryCrash::Update, MomentPlayerStunt's release gate)
    // now reach the base's versions unchanged.

    // The camera this behaviour produced this frame (mLastCamera). The arbitrator states
    // copy it into their own mCamera while the behaviour is driving (the console reaches it
    ...
    // behaviour after SetParameters (byte stores at the offsets noted). Exposed as named
    ...
    // Force every motion-blur pass on for the take (a byte store at +0xE2B). The online-race-intro
    ...
    // (look) vehicle references to one of the intro race cars. The console stores, across
    ...
    // and asserts liRaceCarIndex < 8 (ku8MaxNumRaceCars). That four-field seed IS
    // VehicleRef::SetToRaceCar, so both setters are BODIED 2026-09-12 in this behaviour's own TU
    // as one named call plus the trailing tripwire. Exposed as named setters so the arbitrator
    // state never reaches the (private) refs by offset.
    ...
    // numbered race car. The console writes the same 4-word ref layout with a different kind/index:
    //   ref+0x00 (word)  = 0     (the ref kind: the player ref, not a numbered race car)
    //   ref+0x04 (word)  = -1    (no race-car index)
    //   ref+0x08 (word)  = 0
    //   ref+0x0C (byte)  = 1     (the ref is set / valid)
    // into mPrimaryVehicleRef @+0xDF0 / mSecondaryVehicleRef @+0xE00. Named setters, BODIED
    // 2026-09-12 in this behaviour's own TU through the ref's named fields (the same shape as
    // SetBystanderRefForPostEvent below). FLAG: the ref-kind word's full role (==0 for the
    // player ref) is what the attested store proves.
    ...
    // ⭐ SetCollisionPolicyCanFail -- BODIED 2026-08-01. Every ICE-anim arbitrator state stores
    // the byte 0 into behaviour +0x28 immediately after NewBehaviour<BehaviourIceAnim>:
    //     ArbStateCarSelect::Prepare  (twice, both storing 0)
    //     ArbStateRaceIntro::Update   (storing 0)
    ...
    // mCollisionPolicy sits at +0x20 -- so +0x28 is mCollisionPolicy + 0x08, which the original
    // declarations name VisibilityCollisionPolicy::mbCanFail and which
    // BehaviourIceAnim::Construct seeds to 1 through this same pointer. So the
    ...
    // for mSecondaryVehicleRef @+0xE00. ArbStateCrashing::Update calls exactly
    ...
    // ⚠️ The same operation as SetSecondaryVehicleRefToRaceCarIndex above, reached from a
    // different call site: that one models the inlined four-word {kind=1, index, 0, valid=1}
    // seed (and carries the index tripwire), this one the real out-of-line VehicleRef method,
    ...
    // ADDITIVE GROW (BrnArbStateTakedown.cpp -- SimpleIceTakedownPlayer::Prepare).
    ...
    // (the console calls KeyAnimController::SetParametricTime0To1 on the controller at +0x680
    // with 0.0). Exposed as a named
...
private:
    ...
    // onto the controller (this +0x680) before those stores. They are expressed
    ...
    // FLAG (home): the console keeps this in the BASE's +0x10 slot, which the original names
