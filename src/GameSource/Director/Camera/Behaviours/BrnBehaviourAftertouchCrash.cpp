...
// SetParameters and Get* are defined inline in the header; this .cpp
...
// +0x40 tunable (mfFOV). All three instances reference only the address (lis/addi unk_820051C0
// -- DebugMenu, write, read reuses the same "%s : %f\n" slot), so the
...
// literal bytes when that string is recovered.
